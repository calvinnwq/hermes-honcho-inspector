...
def test_slice_one_install_document_does_not_mutate_a_live_profile() -> None:
    instructions = (ROOT / "INSTALL.md").read_text(encoding="utf-8")

    assert "No release has been published" in instructions
    assert "Do not install Slice 1" in instructions
...
def test_readme_describes_only_the_slice_one_handshake() -> None:
    readme = (ROOT / "README.md").read_text(encoding="utf-8")

    assert "Slice 1" in readme
